...
A central **symbol locator** for the team. Replaces grep. Returns pointers (`repo`, `file`, `line_start`, `line_end`) — never code. Webhook-driven, always fresh, no per-dev clones.
...
```
  ...
  (current code)     │                      │     (FastAPI)        │    │
                     ...
                     │                                                  │
                     │     ┌─────────────────────────────────────┐      │
                     │     │   Generic API MCP (stdio process)   │      │
                     │     │   - reads operations manifest       │      │
                     │     │   - calls REST API over HTTPS       │      │
                     │     └────────────────┬────────────────────┘      │
                     │                      │ stdio (JSON-RPC)          │
                     │                      ▼                           │
                     │              ┌──────────────┐                    │
...
| Secrets (Bitbucket token, HMAC secret) | AWS Secrets Manager | Pulled at startup, never in env files. |
...
- Prometheus `/metrics` endpoint: request counts, latencies (p50/p95/p99), errors, queue depth, parse durations, cache hit rates, Bitbucket quota usage.
- Dashboard with the above plus per-repo freshness.
...
- Incident runbooks in `docs/runbooks/`: webhook outage, worker stuck, SQLite locked, Bitbucket quota exhausted, runaway parse.

**Test**
- Chaos test: kill worker mid-parse, kill REST API for 5 min, send 100 webhooks in 10 seconds. Service recovers without manual intervention. No corruption.
- Backup/restore test: snapshot, wipe staging, restore, run query suite, all passes.
- Cron fallback test: pause webhook for a repo, advance time mock, run cron, confirm reparse triggered.

**Audit**
- Logs scrubbed of secrets — explicit allowlist of fields in the logger config.
...
- Quarterly secret rotation tested in staging.

**Phase gate**
- Chaos test passes end-to-end.
- Restore from backup completes in <15 min and produces working service.
...
- 1-page launch doc + 15-min demo recording with 3 real-world example questions.
...
- Weekly audit log review for first month:
  - Top operations used → keep their manifest text sharp
...
- Adoption metric: ≥ 60% of the 100 devs have called the service ≥ 5 times within 4 weeks of launch. If lower, investigate why before declaring success.

**Audit**
- Token registry: every issued token has a known owner, tracked in an internal sheet or table.
- Token revocation procedure documented and tested at least once.
- Quarterly access review: tokens unused for 90+ days auto-revoked.

**Phase gate**
- Adoption metric met.
- Zero critical incidents in the first 4 weeks.
...
**Total build effort: ~4-5 weeks engineering. Full team active: ~7 weeks calendar.**
...
- A web UI beyond the simple status page

Each of these has merit. None of them block the core value of "Claude stops grepping." Revisit after rollout, based on observed usage patterns from the audit logs.
...
**It does:** return pointers to symbols, callers, dependencies, and file contents.
**It does not:** store code bodies, store metadata, answer questions, or replace Claude.
...
Bitbucket ──webhook──▶ Service (parses on every merge)
                       ...
                       SQLite index
                          ▲
                          │
                       REST API ──▶ Generic API MCP ──▶ Claude Code (your laptop)
```

You don't need to clone any code locally. Claude reaches the service through the team's
existing generic API MCP, configured in shared `.claude/`. See [`docs/USAGE.md`](./docs/USAGE.md)
for how to start using it.
...
| Secrets | AWS Secrets Manager | Bitbucket token, HMAC secret |

## Project layout

```
src/graphify_index/
  api/          # FastAPI routes (locate, code, status, admin)
...
docs/
  ...
  USAGE.md          # how Claude Code users consume the service
